...
def conv_forward_naive(x, w, b, conv_param):
    ...
    I =int(1 + (H + 2 * pad - HH) / stride)
    J =int(1 + (W + 2 * pad - WW) / stride)
    out=np.zeros((N,F,I,J))
    xp=np.pad(x,((0,0),(0,0),(pad,pad),(pad,pad)),mode='constant')
    for i in range(I):
      for j in range(J):
        out[:,:,i,j] = np.sum(xp[:,None,:,i*stride:i*stride+HH,j*stride:j*stride+WW]*w[None,:,:,:,:],axis=(2,3,4)) +b[None,:]
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(I):
      for j in range(J):
        dxp[:,:,i*stride:i*stride+HH,j*stride:j*stride+WW] += np.sum( dout[:,:,i,j][:,:,None,None,None]*w[None,:,:,:,:] ,axis=1)
        dw += np.sum(dout[:,:,i,j][:,:,None,None,None] *xp[:,None,:,i*stride:i*stride+HH,j*stride:j*stride+WW],axis=0) 
